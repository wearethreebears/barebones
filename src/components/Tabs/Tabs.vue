<template>
  <div :data-component="$options.name" :class="part['component']">
    <nav tabindex="0" ref="navigation" data-key="tab-navigation" :class="part['navigation']">
        <ul :class="part['navigationList']">
          <li v-for="(tab, index) in tabs" :key="tab.key" :class="part['navigationListItem']">
            <button
              ...
              ref="key"
              role="tab"
              :class="activeTab == tab.key ? part['navigationButtonActive'] : part['navigationButton']"
            >
              {{ tab.title }}
            </button>
          </li>
