<template>
  <BonesButton ref="modalOpenButton" @click="toggleModal('open')" v-bind="modalOpenProps"><slot name="modalOpenButtonText">Open modal</slot></BonesButton>
  <BonesOverlay 
  ...
  >
    <div 
      ...
      ref="modal"
    >
      <div id='modal-description' class="sr-only"><slot name="SRDescription">This is an open modal window</slot></div>
      <div :class="part['titleWrapper']">
        <h2 id='modal-title' ref="modalTitle" tabindex='0'><slot name="title" /></h2>
      </div>
      <slot />
      <button
...
const modalOpenButton: Ref<typeof BonesButton> = ref(null as unknown as typeof BonesButton)
...
const isVisable: ComputedRef<boolean> = computed(() => state.value !== 'default');

const toggleModal = (newState: string): void => {(state.value = newState)};
